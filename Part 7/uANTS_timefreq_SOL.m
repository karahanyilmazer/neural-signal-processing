...
% Observations: 
%               - Notice the peak amplitude in the 
...
%% An example with real data: Convolve LFP data with Gaussian
...
plot(timevec,signal,'linew',3)
plot(timevec,convres,'r','linew',3)
...
plot(hz,abs(kernX));
plot(hz,abs(convresX)./max(abs(dataX)),'k','linew',2)
...
%% 
% 
%  VIDEO: Making multi-trial convolution more efficient
% 
% 


%% Wavelet convolution for time-frequency analysis
...
subplot(212)
h = plot(timevec,abs(convres),'k','linew',4);
...
%% now for convolution with all trials!
...
%% and now for the awesomeiest thing: construct a time-frequency plane!
...
% IMPORTANT! I'm skipping a few steps of convolution 
...
% specify parameters
circ_prop = .95; % proportion of the circle to fill
...
for i=1:4
    ...
    %%% time domain
    subplot(4,2,i*2-1)
    s = num_cycles(i)/(2*pi*frex); % FWHM of gaussian (don't forget to normalize!)
...
for fi=1:numfrex
    ...
    cmw = exp(2*1i*pi*frex(fi).*wavtime) .* exp( -4*log(2)*wavtime.^2 / .3.^2 );
...
% initialize time-frequency output matrix
% note how we initialize this to the number of *requested* time points!
tf = cell(2,1);
...
for fi=1:numfrex
    ...
    tf{2}(fi,:) = mean(abs(as(times2saveidx,:)).^2,2);
...
% parameters
frex  = linspace(min_freq,max_freq,num_frex);
time  = -1:1/EEG.srate:1; % is this an appropriate time vector for wavelets??!?
half_wave = (length(time)-1)/2;
...
dataXTotal = fft( reshape(EEG.data(chanidx,:,:),1,[]) ,nConv );
dataXNPL   = fft( reshape(EEG.NPL(chanidx,:,:),1,[]) ,nConv );
...
for fi=1:num_frex
    ...
    as = ifft( waveletX.*dataXTotal );
    as = reshape( as(half_wave:end-half_wave-1) ,EEG.pnts,EEG.trials );
    
    % compute power and ITPC
    powr = mean( abs(as).^2 ,2);
    tf(1,fi,:,1) = 10*log10(powr/mean(powr(baseidx(1):baseidx(2))));
    tf(1,fi,:,2) = abs(mean(exp(1i*angle(as)),2));
    
    
    %%% repeat for non-phase-locked activity
    as = ifft( waveletX.*dataXNPL );
    as = reshape( as(half_wave:end-half_wave-1) ,EEG.pnts,EEG.trials );
    
    % compute power and ITPC
    powr = mean( abs(as).^2 ,2);
    tf(2,fi,:,1) = 10*log10(powr/mean(powr(baseidx(1):baseidx(2))));
